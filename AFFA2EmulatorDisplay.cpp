	...
	displayModeChanged(mode);
        ...
        if (ptr < 8) { /* Plus de données */
            ...
            if ((max > 0) && (!fullscreen)) { /* Mode menu */
                ...
                if(selected){
                    AFFA2_LOG(info)<<"update menu selected '"<<text.toStdString()<<"'";
                    emit displayMenuItemUpdate(text, true);
                }else{
		    emit displayMenuItemUpdate(text, false);
                    AFFA2_LOG(info)<<"update menu not selected '"<<text.toStdString()<<"'";
		}
